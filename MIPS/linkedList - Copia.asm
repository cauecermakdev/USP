...
noNextNode:
	...
	lw	$t2, 12($a3)	#pega o endereco da proxima palavra # get address of next field of current node
	sw	$t2, 16($t1)	#coloca em 16($t1), posicao de ponteiro da palavra atual, store that adress in new node's next field 
...
alloSpace:
	...
	li	$a0, 20 #  byte size | allocates 12 bytes, 4 to point to previous, 12 for string and 4 for next
